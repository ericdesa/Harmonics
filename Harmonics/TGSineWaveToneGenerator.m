...

{
    ...
    for (size_t chan = 0; chan < toneGenerator->_numChannels; chan++) {
        ...
        CGFloat absoluteFrequency = toneGenerator->_channels[chan].frequency;
        double theta = toneGenerator->_channels[chan].theta;
        double amplitude = toneGenerator->_channels[chan].amplitude;
        double theta_increment = 2.0 * M_PI * absoluteFrequency / toneGenerator->_sampleRate;
...
//        }
        ...
        CGFloat vibWidth = 0.01f * absoluteFrequency;
        CGFloat vibRate = 1.0f/8.0f;
        
        for (UInt32 frame = 0; frame < inNumberFrames; frame++) {
        
            // guitar
            //buffer[frame] =  (sin(vibRate*theta)) * amplitude;
            ...
            //NSLog(@"%f || %f", buffer[frame], (sin(theta) * amplitude));
        }
...
- (id)initWithChannels:(UInt32)size {
    if (self = [super init]) {
        ...
        for (size_t i = 0; i < _numChannels; i++) {
            _channels[i].frequency = SINE_WAVE_TONE_GENERATOR_FREQUENCY_DEFAULT;
